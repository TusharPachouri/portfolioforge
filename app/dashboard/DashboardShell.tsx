...
import {
  ...
  LayoutGrid, Zap, BarChart2, Moon, Sun, Copy, Check
} from "lucide-react";
  ...
  return (
    <div className={cn("min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col", darkMode && "dark")}>
        ...
        <div className="max-w-screen-xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <button
              ...
              className={cn(
                "press-scale inline-flex items-center gap-2 text-sm font-medium px-3.5 py-1.5 rounded-full transition-colors cursor-pointer disabled:opacity-60",
                published
      ...
      <div className="flex flex-1 max-w-screen-xl mx-auto w-full">
        {/* Sidebar nav — icon rail on small screens, full labels from md up */}
        <aside className="w-14 md:w-52 shrink-0 border-r border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 py-4 px-2 min-h-full flex flex-col">
          <nav className="space-y-4 flex-1" aria-label="Dashboard">
            {NAV_GROUPS.map((group) => (
              <div key={group.label}>
                <p className="hidden md:block px-3 mb-1.5 text-[10px] font-semibold tracking-[0.18em] text-zinc-400 dark:text-zinc-500 uppercase select-none">
                  {group.label}
                </p>
                  ...
                  {group.items.map(({ href, label, icon: Icon, proOnly }) => {
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          "relative flex items-center justify-center md:justify-start gap-2.5 px-2 md:px-3 py-2 rounded-lg text-sm font-medium transition-colors",
                          active
                      ...
                      >
                        {active && (
                          <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-0.5 rounded-full bg-violet-400 hidden md:block" aria-hidden="true" />
                        )}
                        <Icon className="h-4 w-4 shrink-0" aria-hidden="true" />
                        <span className="hidden md:inline flex-1">{label}</span>
                        {locked && (
                          <span className="hidden md:inline-flex items-center gap-1 text-[10px] font-bold text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/60 px-1.5 py-0.5 rounded-full">
          ...
          {!isProUser && (
            <div className="hidden md:block mt-4 mx-1">
